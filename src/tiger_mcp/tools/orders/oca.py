...
- ``preview_oca_order``     -- dry-run an OCA SELL order (take-profit +
  stop-loss) to protect an existing long position.
- ``place_oca_order``       -- submit an OCA SELL order after running all
  safety checks.
- ``preview_bracket_order`` -- dry-run a bracket BUY order (entry + TP + SL).
...
async def preview_oca_order(
    ...
    """Preview an OCA (One-Cancels-All) SELL order without executing it.
    ...
    runs all safety checks, and fetches a cost estimate from the broker.
    ...
    # 4. Fetch cost preview from broker
    try:
        preview = await _client.preview_oca_order(
            symbol=symbol,
            quantity=quantity,
        ...
        return f"Error previewing OCA order: {exc}"
    ...
    lines: list[str] = [
        ...
        "  Action:            SELL",
        f"  Quantity:          {quantity}",
        f"  TP Limit Price:    ${tp_limit_price:,.2f}",
        f"  SL Stop Price:     ${sl_stop_price:,.2f}",
        f"  SL Limit Price:    ${sl_limit_price:,.2f}",
        "",
    ]

    if isinstance(estimated_cost, (int, float)):
        lines.append(f"  Estimated Cost:    ${estimated_cost:,.2f}")
    else:
...
async def preview_bracket_order(
    ...
    """Preview a bracket BUY order without executing it.
    ...
    Validates parameters, runs all safety checks, and fetches a cost
    estimate from the broker.  The order is NOT submitted.
    ...
    try:
        preview = await _client.preview_bracket_order(
            symbol=symbol,
            quantity=quantity,
        ...
        return f"Error previewing bracket order: {exc}"
    ...
    lines: list[str] = [
        ...
        "  Action:            BUY",
        f"  Quantity:          {quantity}",
        f"  Entry Price:       ${entry_limit_price:,.2f}",
        f"  TP Limit Price:    ${tp_limit_price:,.2f}",
        f"  SL Stop Price:     ${sl_stop_price:,.2f}",
        f"  SL Limit Price:    ${sl_limit_price:,.2f}",
        "",
